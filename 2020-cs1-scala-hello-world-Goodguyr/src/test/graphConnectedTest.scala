...
class graphConnectedTest extends FunSuite {

  test("Testing connected nodes"){
    ...
    assert(graph.areConnected(2,5))

  }

  test("Testing disconnected nodes"){
    val graph: Graph[String] = new Graph()
    graph.addNode(0, "UCLA")
    graph.addNode(1, "STANFORD")
    graph.addNode(2, "SRI")
